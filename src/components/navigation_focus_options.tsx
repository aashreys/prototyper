import { Bold, Dropdown, DropdownOption, Text, Textbox, TextboxColor, TextboxNumeric, VerticalSpace } from '@create-figma-plugin/ui'
import { Component, h } from 'preact'
import { NavigationFocusConfig, NavigationFocusMode, ShadowFocusConfig, StrokeFocusConfig } from '../navigation_focus'
...
const MODE_OPTIONS: Array<DropdownOption> = [
  { value: NavigationFocusMode.VARIANT, text: 'Variant' },
  { value: NavigationFocusMode.STROKE, text: 'Stroke' },
  { value: NavigationFocusMode.SHADOW, text: 'Shadow' }
]

export class NavigationFocusOptions extends Component<NavigationFocusOptionsProps, any>  {
    ...
    return (
      <div style='display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 6px;'>
        <div style='grid-column: 1 / -1;'>
          ...
          onHexColorInput={e => this.onStrokeColorChange(e.currentTarget.value)}
          opacity="100" />
        </div>

        {this.renderNumericControl('Weight', props.focus.stroke.weight, value => this.onStrokeNumberChange('weight', value))}
        {this.renderNumericControl('Padding', props.focus.stroke.padding, value => this.onStrokeNumberChange('padding', value))}
        {this.renderNumericControl('Corner radius', props.focus.stroke.cornerRadius, value => this.onStrokeNumberChange('cornerRadius', value))}
      </div>
    )
  }
    ...
    return (
      <div style='display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 6px;'>
        <div style='grid-column: 1 / -1;'>
          ...
          onHexColorInput={e => this.onShadowColorChange(e.currentTarget.value)}
          opacity="100" />
        </div>

        {this.renderNumericControl('Blur', props.focus.shadow.blur, value => this.onShadowNumberChange('blur', value))}
        {this.renderNumericControl('Spread', props.focus.shadow.spread, value => this.onShadowNumberChange('spread', value))}
        {this.renderNumericControl('Padding', props.focus.shadow.padding, value => this.onShadowNumberChange('padding', value))}
        {this.renderNumericControl('Corner radius', props.focus.shadow.cornerRadius, value => this.onShadowNumberChange('cornerRadius', value))}
      </div>
    )
  }

  renderNumericControl(label: string, value: number, onChange: (value: null | number) => void) {
    return (
      <div>
        <Text>{label}</Text>
        <TextboxNumeric
  ...
  renderFocusModeControls(props: NavigationFocusOptionsProps) {
    if (props.focus.mode === NavigationFocusMode.VARIANT) return this.renderVariantControls(props)
    if (props.focus.mode === NavigationFocusMode.SHADOW) return this.renderShadowControls(props)
  ...
  render(props: NavigationFocusOptionsProps, _state) {
    return (
      <div style={props.style ? props.style : ''}>
        ...
        <Dropdown
        onChange={e => this.onModeChange(e.currentTarget.value as NavigationFocusMode)}
